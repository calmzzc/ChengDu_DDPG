...
def train(cfg, line, agent, train_model):
    ...
    for i_ep in range(cfg.train_eps):
        ...
        while True:
            ...
            node_list.append(state_node)
        if (i_ep + 1) % 10 == 0:
            print('回合：{}/{}，奖励：{}, 能耗  {}, 最终时间  {}, 最终速度  {}, 不安全次数  {}'.format(i_ep + 1,
                                                                                 cfg.train_eps,
                                                                                 np.around(ep_reward,
                                                                                           2),
                                                                                 np.around(total_power,
                                                                                           2),
                                                                                 np.around(
                                                                                     state_node.state[0],
                                                                                     2), np.
                                                                                 around(
                    state_node.state[1], 2),
...
if __name__ == "__main__":
    ...
    rewards, ma_rewards, v_list, t_list, a_list, ep_list, power_list, ma_power_list, unsafe_c, ma_unsafe_c, total_acc_list, total_t_power_list, total_re_power_list = train(cfg, line, agent, train_model)
