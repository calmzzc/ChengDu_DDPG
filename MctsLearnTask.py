...
# 集成式计算每一幕的过程，可直接运行，要注意测试时Additional Learner的维度
# 注意使用的是哪个Agent
...
if __name__ == "__main__":
    ...
    for j in range(1, 17):
        ...
        for i in range(1,6):
            ...
            # 测试
            line, agent, train_mdoel = env_agent_config(cfg, seed=random_seed + i)
        ...
        with open(section + 'ddpg_' + 't_protect.csv', 'w', newline='') as file:
            writer = csv.writer(file)
            # 遍历嵌套列表中的每个子列表，并将其写入CSV文件
            for sublist in t_t_protect_counts:
                writer.writerow(sublist)
        with open(section + 'ddpg_' + 'e_protect.csv', 'w', newline='') as file:
            writer = csv.writer(file)
            # 遍历嵌套列表中的每个子列表，并将其写入CSV文件
            for sublist in e_t_protect_counts:
                writer.writerow(sublist)
        with open(section + 'ddpg_' + 'e_en.csv', 'w', newline='') as file:
            writer = csv.writer(file)
            # 遍历嵌套列表中的每个子列表，并将其写入CSV文件
            for sublist in e_t_en_list:
                writer.writerow(sublist)
        with open(section + 'ddpg_' + 'e_ti.csv', 'w', newline='') as file:
            writer = csv.writer(file)
            # 遍历嵌套列表中的每个子列表，并将其写入CSV文件
            for sublist in e_t_ti_list:
                writer.writerow(sublist)

        with open(section + 'ddpg_' + 't_mar.csv', 'w', newline='') as file:
            writer = csv.writer(file)
            # 遍历嵌套列表中的每个子列表，并将其写入CSV文件
